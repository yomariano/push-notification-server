...
// Dedicated backend for handling push notifications via OneSignal and Web Push API
...
app.use(cors({
  origin: [
    'https://react.signalstrading.app',
    'https://signalstrading.app',
    'http://localhost:5173',
    'http://localhost:3000'
  ],
  credentials: true
...
console.log('🔐 OneSignal API Key:', ONESIGNAL_API_KEY ? 'Loaded' : 'Missing!');
...
app.get('/api/health', (req, res) => {
  res.json({
    ...
    endpoints: [
      ...
      'POST /api/send-notification - Send Web Push',
      'POST /api/onesignal-send - Send to specific OneSignal user',
      'POST /api/onesignal-broadcast - Broadcast to all OneSignal users',
      'POST /api/trading-signal - Send trading signal (with conditions)',
      'POST /api/price-alert - Send price alert (with conditions)',  
      'POST /api/market-event - Send market event (with conditions)',
...
// API endpoint to send push notification via Web Push API
app.post('/api/send-notification', async (req, res) => {
  ...
  const payload = JSON.stringify({
    ...
    body: body || 'Test notification from server!',
    icon: icon || '/vite.svg',
    ...
    data: {
      url: '/',
      timestamp: Date.now(),
      source: 'push-notification-server',
      ...data
    },
...
app.listen(PORT, '0.0.0.0', () => {
  ...
  console.log(`   GET  /api/vapid-key - Get VAPID public key`);
  console.log(`   POST /api/subscribe - Save Web Push subscription`);
  console.log(`   POST /api/send-notification - Send Web Push notification`);
  console.log(`   POST /api/onesignal-send - Send to OneSignal user`);
  console.log(`   POST /api/onesignal-broadcast - Broadcast to all OneSignal users`);
  console.log(`   POST /api/trading-signal - Send conditional trading signals`);
  console.log(`   POST /api/price-alert - Send conditional price alerts`);
  console.log(`   POST /api/market-event - Send conditional market events`);
  console.log(`   GET  /api/subscriptions - View subscriptions`);
  console.log(`   DELETE /api/subscriptions - Clear subscriptions`);
  console.log('');
  console.log('🎯 Ready to send push notifications programmatically!');
  console.log('🌐 CORS enabled for: https://react.signalstrading.app');
});

// Graceful shutdown
process.on('SIGTERM', () => {
  console.log('📴 Push Notification Server shutting down gracefully');
  process.exit(0);
});
